...
export function RoundSessionFlow() {
  ...
  const totalSteps = ROUND_QUESTIONS.length
  const isLastStep = step === ROUND_QUESTIONS.length - 1
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const payload = { questions: answerDetails }
    ...
    <div className="app-screen">
      <header className="app-header">
        <div>
          ...
          <h1>Question {step + 1} of {ROUND_QUESTIONS.length}</h1>
        </div>
      </header>
      ...
      >
        {isLastStep ? (submitting ? 'Generating your recap…' : 'Finish session') : 'Next'}
      </button>
